...
jQuery(document).ready(function(){
	...
	if (jQuery('.hourlabel')){
		hrlbl = jQuery('.hourlabel').attr('rel');
	}

	if (jQuery('.minutelabel')){
		mntlbl = jQuery('.minutelabel').attr('rel');
	}
	

	jQuery(document).on("click", ".daynumber", function(){ 
		var parentski = jQuery(this).parents('table');
		var calbox = parentski.parents('.cro_sidebox').find('input');
		var calval = calbox.val();
		var clickval = jQuery(this).attr('rel');
		var currday = parentski.find('.thisday');
		jQuery('.thisday').toggleClass('thisday');
		jQuery(this).parent('span').addClass('thisday');
		calbox.val(clickval);
	});

	// CALENDAR FUNCTIONS
	jQuery(document).on("click", ".caldir", function(){ 
		var timings = jQuery(this).attr('rel');
		var parentski = jQuery(this).parents('table');
		var calbox = parentski.parents('.cro_sidebox').find('input');
		var calval = calbox.val();
		var data = { action: 'crombox_post_action', type: 'cro_movecal', option1: timings, option2: calval};
		jQuery.post(ajaxurl, data, function(response) {
			jQuery('.cro_calholder').html(response);	
		});
	});

	jQuery('#noUiSlider').noUiSlider('init', {
		handles: 1,
		connect: "lower",
		scale: [0,23],
		start: hrlbl,
		change:    
			function(){
				...
				jQuery(this).find('.noUi-upperHandle .infoBox').text(btn);
			},
		end:
			function() {
				var values = jQuery(this).noUiSlider( 'value' );
				...
				if (jQuery('.timelabel').hasClass('cro_timeres')) {
					mntlbal = parseInt(mntlbl, 10);
					jQuery('.timelabel').val(Math.floor(((values[1]*60) + mntlbal))*60);
					alert(mntlbl);
				} else {
					jQuery('.timelabel').val(hrlbl + ':' + mntlbl);
				}
			}
	}).find('.noUi-handle div').each(function(index){
		var values = jQuery(this).parent().parent().noUiSlider( 'value' );
		if (values[1] <= 9) {
			var btn = '0' + values[1];
		}else {
			var btn = values[1];
		}	
		jQuery(this).append('<span class="infoBox">'+btn+'</span>');

	});

	jQuery('#noUiSlider2').noUiSlider('init', {
		handles: 1,
		connect: "lower",
		scale: [0,59],
		start: mntlbl,
		change:    
			...
			function() {
				...
				if (jQuery('.timelabel').hasClass('cro_timeres')) {
					hrlbal = parseInt(hrlbl, 10);
					jQuery('.timelabel').val(Math.floor(((hrlbal*60) + values[1]))*60);
					alert(mntlbl);
				} else {
					jQuery('.timelabel').val(hrlbl + ':' + mntlbl);
				}
	...
	jQuery('.optspan').click(function() {
		var relval = jQuery(this).attr('rel');
		var curpar = jQuery(this).parents('.calopti');
		var curchil = curpar.find('.optspanselected');

		if (relval != 1) {
			curpar.find('.showlastday').removeClass('showdaynone');
		} else {
			curpar.find('.showlastday').addClass('showdaynone');
		}

		if (relval != 5) {
			curpar.find('.showadvconf').addClass('showdaynone');
		} else {
			curpar.find('.showadvconf').removeClass('showdaynone');
		}

		curchil.removeClass('optspanselected');
		jQuery(this).addClass('optspanselected');
		jQuery('.crocalboxx').val(relval);
